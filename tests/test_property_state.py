...
For any combination of valid settings (font family from available fonts,
font size 10-120, hex color string, vertical position 0-100, arbitrary
subject/body strings), persisting all and loading back produces identical
values.
...
class TestProperty14SettingsRoundTrip:
    """Property 14: Persist valid settings and load back produces identical values."""
    ...
    @given(
        ...
        email_body=st.text(min_size=0, max_size=500),
    )
    @settings(max_examples=100)
    def test_all_settings_roundtrip(
        self,
        font_family: str,
        font_size: int,
        font_color: str,
        ...
        # Feature: offline-cross-platform-app, Property 14: Settings round-trip
    ...
    @settings(max_examples=100)
    def test_numeric_values_survive_string_conversion(
        self, font_size: int, vertical_position: int
    ) -> None:
        """Numeric values stored as strings convert back correctly."""
        # Feature: offline-cross-platform-app, Property 14: Settings round-trip
        page = _create_mock_page()
        manager = AppStateManager(page)
